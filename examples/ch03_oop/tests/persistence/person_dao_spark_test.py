...
def test_find_all_args_supplied() -> None:
    ...
    assert (1, "Vivien", "Theodore", "Thomas") == \
           (p.id, p.given_name, p.middle_name, p.family_name)
...
def test_find_several_queries() -> None:
    ...
    assert ("Vivien", "Theodore", "Thomas") == (p.given_name, p.middle_name, p.family_name)

    p = next(iter(dao.find("Elizabeth", family="Blackwell")))
    assert ("Elizabeth", "", "Blackwell") == (p.given_name, p.middle_name, p.family_name)

    p = next(iter(dao.find("Hippocrates")))
    assert ("Hippocrates", "", "") == (p.given_name, p.middle_name, p.family_name)
...
def test_find_by_id_success() -> None:
    ...
    assert "Hippocrates" == person.given_name
