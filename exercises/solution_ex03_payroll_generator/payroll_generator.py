...
class EmployeeDao:
    ...
    def update_employee_record(self, emp_id, gross_pay, total_deductions):
...
class PayCalculator:
    ...
    def calculate_net_pay(self, pay_record):
        emp_id = pay_record['id']
        gross_pay = self.calculate_gross_pay(emp_id, pay_record['hours_worked'])
        total_deductions = self.calculate_deductions(
            emp_id, gross_pay, pay_record['tax_status'])
        self.employee_dao.update_employee_record(
            emp_id, gross_pay, total_deductions)
        net_pay = gross_pay - total_deductions

        return emp_id, round(gross_pay, 2), \
            round(total_deductions, 2), round(net_pay, 2)

    def calculate_gross_pay(self, emp_id, hours_worked):
        # should lookup hourly rate from database
        return hours_worked * (15.0 + 75 * random())

    def calculate_deductions(self, emp_id, gross, tax_status):
        tax_withheld = self.calculate_tax_withheld(emp_id, gross, tax_status)
        ss_withheld = self.calculate_ss_withheld(gross)
        mc_withheld = self.calculate_mc_withheld(gross)
        return tax_withheld + ss_withheld + mc_withheld

    def calculate_tax_withheld(self, emp_id, gross_pay, tax_status):
        # should get tax rate based on tax status
        tax_withheld = gross_pay * (0.10 + 0.15 * random())
        return tax_withheld

    def calculate_ss_withheld(self, gross_pay):
        ss_withheld = gross_pay * ss_tax_rate
        return ss_withheld

    def calculate_mc_withheld(self, gross_pay):
...
class PayrollProcessor:
    ...
    def __init__(self):
        ...
        self.check_printer = CheckPrinter()
